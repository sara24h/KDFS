...
warmup_start_lr=${WARMUP_START_LR:-4e-5}
lr_decay_T_max=${LR_DECAY_T_MAX:-250}
lr_decay_eta_min=${LR_DECAY_ETA_MIN:-4e-5}
...
coef_kdloss=${COEF_KDLOSS:-0.05}
coef_rcloss=${COEF_RCLOSS:-1.0}
compress_rate=${COMPRESS_RATE:-0.6}
finetune_num_epochs=${FINETUNE_NUM_EPOCHS:-6}
finetune_lr=${FINETUNE_LR:-4e-6}
finetune_warmup_steps=${FINETUNE_WARMUP_STEPS:-5}
finetune_warmup_start_lr=${FINETUNE_WARMUP_START_LR:-4e-8}
finetune_lr_decay_T_max=${FINETUNE_LR_DECAY_T_MAX:-20}
finetune_lr_decay_eta_min=${FINETUNE_LR_DECAY_ETA_MIN:-4e-8}
finetune_weight_decay=${FINETUNE_WEIGHT_DECAY:-2e-5}
