...
class Train:
    ...
    def result_init(self):
        ...
        self.logger = utils.get_logger(
            os.path.join(self.result_dir, f"{self.phase}_logger.log"), f"{self.phase}_logger"
        )
    ...
    def build_model(self):
        ...
        if self.phase == 'train':
            ...
            self.teacher = eval(f"{self.arch}_hardfakevsreal")()
        ...
        self.student = eval(f"{self.arch}_sparse_{self.dataset_type}")(
    ...
    def define_optim(self):
        ...
        mask_params = [
            p[1] for p in self.student.named_parameters() if p[1].requires_grad and "mask" in p[0]
        ]
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch + 1, self.num_epochs + 1):
            ...
            self.writer.add_scalar(
                "train/temperature/gumbel_temperature", self.student.gumbel_temperature, global_step=epoch
            )
            self.writer.add_scalar("train/Flops", Flops, global_step=epoch)
            log_msg = (
                f"[{self.phase.capitalize()}] Epoch {epoch} : "
            ...
            log_msg += f"TotalLoss {meter_loss.avg:.4f} Prec@1 {meter_top1.avg:.2f}"
            ...
            self.logger.info(
                f" => Best top1 accuracy before {'finetune' if self.phase == 'train' else 'final'} : {self.best_prec1}"
            )
